...
describe('normalizeExpression', () => {
	...
	describe('Parentheses Normalization', () => {
		...
		it('should remove outer parentheses around entire right-hand side', () => {
			expect(normalizeExpression('Q = ((A AND B) OR C)')).toBe('Q = (A AND B) OR C')
	...
	describe('NOT Expression Normalization', () => {
		it('should remove double parentheses around NOT expressions', () => {
			expect(normalizeExpression('Y = F OR ((NOT D) AND G)')).toBe('Y = F OR (NOT D AND G)')
		})

		it('should normalize outer parentheses around NOT expressions', () => {
			expect(normalizeExpression('X = (NOT (A AND B))')).toBe('X = NOT (A AND B)')
		})
		...
		it('should handle complex NOT expressions with extra spaces', () => {
			expect(normalizeExpression('Z = ( NOT ( A AND B ) ) OR C')).toBe('Z =(NOT (A AND B)) OR C')
			expect(normalizeExpression('Z = (NOT (A AND B)) OR C')).toBe('Z =(NOT (A AND B)) OR C')
		})
	})
...
describe('getInputVariables', () => {
	...
	it('should extract variables from complex expression', () => {
		expect(getInputVariables('Q = (A AND B) OR (C AND D)')).toEqual(['A', 'B', 'C', 'D'])
	})
...
describe('evaluateExpression', () => {
	...
	it('should evaluate complex expression with parentheses', () => {
		expect(evaluateExpression('(A AND B) OR C', { A: true, B: true, C: false })).toBe(true)
		expect(evaluateExpression('(A AND B) OR C', { A: false, B: true, C: false })).toBe(false)
		expect(evaluateExpression('(A AND B) OR C', { A: false, B: false, C: true })).toBe(true)
	})

	it('should evaluate NOT with complex expressions', () => {
		expect(evaluateExpression('NOT (A AND B)', { A: true, B: true })).toBe(false)
		expect(evaluateExpression('NOT (A AND B)', { A: true, B: false })).toBe(true)
	})

	it('should handle nested expressions', () => {
		expect(evaluateExpression('(A OR B) AND (C OR D)', { 
...
describe('areExpressionsLogicallyEquivalent', () => {
	it('should recognize identical expressions', () => {
		expect(areExpressionsLogicallyEquivalent('Q = A AND B', 'Q = A AND B')).toBe(true)
	})

	it('should recognize commutative equivalence', () => {
		expect(areExpressionsLogicallyEquivalent('Q = A AND B', 'Q = B AND A')).toBe(true)
		expect(areExpressionsLogicallyEquivalent('Q = A OR B', 'Q = B OR A')).toBe(true)
	})

	it('should recognize De Morgan\'s law equivalences', () => {
		expect(areExpressionsLogicallyEquivalent(
			'Q = NOT (A AND B)',
			'Q = (NOT A) OR (NOT B)'
		)).toBe(true)
		expect(areExpressionsLogicallyEquivalent(
	...
	it('should recognize double negation', () => {
		expect(areExpressionsLogicallyEquivalent('Q = NOT (NOT A)', 'Q = A')).toBe(true)
	})

	it('should detect non-equivalent expressions', () => {
		expect(areExpressionsLogicallyEquivalent('Q = A AND B', 'Q = A OR B')).toBe(false)
