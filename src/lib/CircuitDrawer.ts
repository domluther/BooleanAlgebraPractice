...
export class CircuitDrawer {
  ...
  private gateImages: Record<GateType, HTMLImageElement> = {} as Record<GateType, HTMLImageElement>;
  ...
  private parsedTargetExpression: ParsedExpression = { output: 'Q', inputs: ['A', 'B'] };
   ...
   */
  start(expression: string, interpretedExprElement: HTMLElement | null, difficulty = 1): void {
  ...
  private _addCircuitModeEventListeners(): void {
    ...
    document.querySelectorAll('.gate[data-gate-type]').forEach(gate => {
      gate.addEventListener('click', (e) => {
        e.preventDefault();
        const gateType = (gate as HTMLElement).dataset.gateType || 
    ...
    document.querySelectorAll('.gate[draggable="true"]').forEach(gate => {
      gate.addEventListener('dragstart', (e) => {
        ...
        if (dragEvent.dataTransfer) {
          dragEvent.dataTransfer.setData('text/plain', (e.currentTarget as HTMLElement).id);
          const svg = (e.currentTarget as HTMLElement).querySelector('img');
          if (svg) {
            dragEvent.dataTransfer.setDragImage(svg, svg.width / 2, svg.height / 2);
          }
        }
      }, { signal });
    });
    ...
    this.canvas.addEventListener('dragover', (e) => e.preventDefault(), { signal });
    this.canvas.addEventListener('drop', (e) => {
      ...
      this._addGate(type, dragEvent.clientX - rect.left, dragEvent.clientY - rect.top);
      this._clearGateSelection();
    }, { signal });
  ...
  private _updateInterpretedExpression(): void {
    ...
    if (this.interpretedExprElement) {
      this.interpretedExprElement.textContent = convertToNotation(this.interpretedExpression, this.notationType);
    }
  ...
  private _findNodeByConnectionInfo(connection: ConnectionInfo): Node | null {
    ...
    if (!gate) return null;
    return nodeType === 'input' && nodeIndex !== undefined ? gate.inputNodes[nodeIndex] : gate.outputNode;
  }
