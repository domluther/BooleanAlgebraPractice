...
export class CircuitGenerator {
	...
	layoutNodes(
		...
		if (node.type === 'VAR') {
			...
			return {
				...
				width: 80,
				height: 60,
				left: this.layoutNodes(binaryNode.left, x - 100, leftY, level + 1, y - 15)!,
				right: this.layoutNodes(binaryNode.right, x - 100, rightY, level + 1, y + 15)!
	...
	getCircuitDepth(node: ASTNode | LayoutNode | null): number {
		...
		const binaryNode = node as BinaryOpNode | BinaryOpLayoutNode
		return 1 + Math.max(this.getCircuitDepth(binaryNode.left), this.getCircuitDepth(binaryNode.right))
	}
