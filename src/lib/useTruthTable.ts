...
interface UseTruthTableReturn {
	...
	setUserAnswer: (rowIndex: number, columnName: string, value: "0" | "1" | "") => void;
	checkAnswer: () => void;
	generateNewQuestion: () => void;
}

export function useTruthTable({ onScoreUpdate }: UseTruthTableProps = {}): UseTruthTableReturn {
	...
	const [userAnswers, setUserAnswers] = useState<Map<string, "0" | "1" | "">>(new Map());
	const [cellValidations, setCellValidations] = useState<Map<string, CellValidation>>(new Map());
	 ...
	 */
	const setUserAnswer = useCallback((rowIndex: number, columnName: string, value: "0" | "1" | "") => {
	...
	}, [isAnswered, expertMode, userAnswers, truthTableData, outputVariable, inputs, intermediateExpressions]);
	...
	const checkNormalModeAnswer = useCallback(() => {
		...
		if (!allOutputAnswered) {
			setFeedbackMessage(`Please answer all rows in the output column (${outputVariable}).`);
			return;
		}
		...
		if (correct) {
			...
			if (onScoreUpdate) {
				onScoreUpdate(true, `truthTable_level${currentLevel}`);
			}
		} else {
			setFeedbackMessage(`Output column: ${outputCorrect}/${truthTableData.length} correct. Review the highlighted answers.`);
			// Allow retry - don't mark as answered
			setIsAnswered(false);
			if (onScoreUpdate) {
				onScoreUpdate(false, `truthTable_level${currentLevel}`);
			}
		}
	}, [userAnswers, truthTableData, outputVariable, showIntermediateColumns, intermediateExpressions, currentLevel, onScoreUpdate]);
	...
	const checkExpertModeAnswer = useCallback(() => {
		...
		for (let rowIndex = 0; rowIndex < truthTableData.length; rowIndex++) {
			...
			for (const input of inputs) {
				...
				if (userValue === undefined || userValue === "") {
					...
					validations.set(key, { rowIndex, columnName: input, status: "unanswered" });
				} else {
			...
			if (showIntermediateColumns) {
				for (let i = 0; i < intermediateExpressions.length; i++) {
					...
					if (userValue === undefined || userValue === "") {
						allFieldsFilled = false;
						validations.set(key, { rowIndex, columnName, status: "unanswered" });
					} else {
			...
			if (outputValue === undefined || outputValue === "") {
				allFieldsFilled = false;
				validations.set(outputKey, { rowIndex, columnName: outputVariable, status: "unanswered" });
			} else {
		...
		const validationResult = validateExpertModeAnswers(userRows, truthTableData);
		...
		for (let rowIndex = 0; rowIndex < truthTableData.length; rowIndex++) {
			...
			const allColumns = [
				...inputs,
				...(showIntermediateColumns ? intermediateExpressions.map((_, i) => `intermediate_${i}`) : []),
				outputVariable
			];
		...
		if (validationResult.isCorrect) {
			...
			if (onScoreUpdate) {
				onScoreUpdate(true, `truthTable_level${currentLevel}_expert`);
			}
		} else {
			setFeedbackMessage(`Expert Mode: ${validationResult.correctRows}/${truthTableData.length} rows correct.`);
			if (onScoreUpdate) {
				onScoreUpdate(false, `truthTable_level${currentLevel}_expert`);
			}
		}
	}, [userAnswers, truthTableData, inputs, intermediateExpressions, outputVariable, showIntermediateColumns, currentLevel, onScoreUpdate]);
	 ...
	 */
	const handleSetShowIntermediateColumns = useCallback((show: boolean) => {
